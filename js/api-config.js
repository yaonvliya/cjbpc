...
var commonApiUrl = {
	//获取省份信息(get)
	getProvince: apiHost +"/core/region/provinces",

	//获取城市信息（需要拼接省份code）(get)
	getCity: apiHost +"/core/region/cities/",

	//获取区(县)信息（需要拼接城市code）(get)
	getAreas: apiHost +"/core/region/areas/",

	//获取银行列表(get)
	getBank: apiHost +"/front/user/deposit/deposit_support_banks",

	//获取支行名称
	getBankBranch: apiHost +"/core/bank/branch",
...
var captchaApiUrl = {
	...
	// 获取图片验证码(get)
	getImgCode: apiHost + "/front/common/captcha/image",

	// 获取短信验证码 需传入手机号码、图片验证码参数
	getSmsCode: apiHost +"/front/common/captcha/mobile/input",

	// 获取邮箱验证码 需传入邮箱、图片验证码参数
	getMailCode: apiHost +"/front/common/captcha/email/input",

	//获取短信验证码（登录账户） 不需传入手机号码、图片验证码参数
	getSmsCodeBindMobile: apiHost +"/front/common/captcha/mobile/bind",

	// 获取邮箱验证码（登录账户） 不需传入邮箱、图片验证码参数
	getMailCodeBindEmail: apiHost +"/front/common/captcha/email/bind"

};

var depositApiUrl = {
    //获取存管API上下文
    depositApiContext: apiHost + "/front/user/deposit/page_api_params",

    //获取存管API调用结果
    depositResult: apiHost + "/front/user/deposit/page_api_result"
};
